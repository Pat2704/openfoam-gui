 ...
 * build-electron.js
 *
 * Single entry point for producing OpenFOAMStudio-v1.1-portable.exe.
 *
 ...
 *
 * Output: dist-electron/OpenFOAMStudio-v1.1-portable.exe
 *
 * Usage:
...
function main() {
  ...
  const out = path.join(ROOT, "dist-electron", "OpenFOAMStudio-v1.1-portable.exe");
  if (fs.existsSync(out)) {
    const mb = (fs.statSync(out).size / 1048576).toFixed(1);
    log(`SUCCESS: dist-electron/OpenFOAMStudio-v1.1-portable.exe (${mb} MB)`);
  } else {
