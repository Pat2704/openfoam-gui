 ...
 * build-electron.js
 *
 * Single entry point for producing OpenFOAMStudio-v1.2-portable.exe.
 *
 ...
 *
 * Output: dist-electron/OpenFOAMStudio-v1.2-portable.exe
 *
 * Usage:
...
function main() {
  ...
  const out = path.join(ROOT, "dist-electron", "OpenFOAMStudio-v1.2-portable.exe");
  if (fs.existsSync(out)) {
    const mb = (fs.statSync(out).size / 1048576).toFixed(1);
    log(`SUCCESS: dist-electron/OpenFOAMStudio-v1.2-portable.exe (${mb} MB)`);
  } else {
