...
// electron/ has a lock file of its own with the same two top-level fields. It
// is GITIGNORED and regenerated by electron-builder from electron/package.json
// on every build, so it corrects itself and is not part of a version bump —
// but a stale one left in a working tree is confusing to read, and this says so
// rather than leaving it to be puzzled over. Absent on a fresh clone, which is
// normal and not a problem.
const electronLockPath = path.join(ROOT, "electron/package-lock.json");
if (fs.existsSync(electronLockPath)) {
  ...
  if (electronLock.version !== version || electronLock.packages[""].version !== version) {
    problems.push(
      `electron/package-lock.json is ${electronLock.version} / ${electronLock.packages[""].version}, ` +
        `expected ${version} — it is gitignored and the next build regenerates it, so this is stale, not fatal`
    );
  }
}
