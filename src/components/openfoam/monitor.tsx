...
export default function Monitor({ caseName }: { caseName: string }) {
  ...
  // ── CheckMesh state ──
  const [checkMeshResult, setCheckMeshResult] = useState<{
    success: boolean;
    raw: string;
  ...
  const validateBCAction = useCallback(async () => {
    ...
    try {
      ...
      if (!data.success) {
        ...
        const totalPatches = data.fields.reduce((s: number, f: any) => s + f.patches.length, 0);
        const invalidPatches = data.fields.reduce((s: number, f: any) => s + f.patches.filter((p: any) => !p.valid).length, 0);
