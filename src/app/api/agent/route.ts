...
function buildSystemPrompt(version: string, caseName: string, unrestricted: boolean): string {
  if (unrestricted) {
    return [
      ...
      'run_openfoam is now a real shell inside the case directory: any command, pipes, redirects, chaining — nothing is',
      'filtered, and nothing will stop you. Deleting, moving and overwriting all work now.',
      '',
