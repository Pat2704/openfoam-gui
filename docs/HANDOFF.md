...
Last updated: 2026-09-03, after cutting v2.3.0.

---

## Where things stand right now (read this first)

**v2.3.0 is released.** Everything is committed and pushed to `main`, the tag is
...
**The `v2.3.0` tag builds exactly the artifacts that are attached.** It was put
on the build commit, not moved onto it afterwards — `git diff v2.3.0 HEAD` was
empty at the moment of tagging. Keep doing it in that order and the check stays
a one-liner.

| | |
|---|---|
| release commit | `1d01a85` — tagged `v2.3.0`, what the attached artifacts were built from |
| tags | …`v2.2.0` at `2824fe5`, `v2.2.1` at `d0baf7f`, `v2.3.0` at `1d01a85` |
| latest release | https://github.com/Pat2704/openfoam-gui/releases/tag/v2.3.0 — both artifacts attached |
| in `Working/` | the checkout, `OpenFOAMStudio-source/`, the two artifacts `OpenFOAMStudio-v2.3.0-{portable.exe,folder.zip}`, and `OpenCFD-trademark-request.md` (§2l). Release notes live in the repo, `docs/releases/` (§4b) |
...
- **Foreground output STREAMS now.** `executeCommandAsync` had always taken an
  ...
  The catalogue GET and the non-stream POST are untouched — only the panel
  passes `stream`.
- **The flush is on an 80 ms TIMER, not requestAnimationFrame.** rAF was the
  ...
  remove. Measured: with rAF the transcript stayed empty for a hidden window,
  with the timer it grows as the output arrives.
- **Colour is per LINE, not per block.** The block used to go entirely red on a
  ...
  dropped for plain text — a 5 MiB log is 100k lines and does not need 100k
  elements.
...
- **The worker wrote its PID AFTER sourcing the OpenFOAM bashrc.** That source
  ...
  the source; `exec` preserves it, so it stays valid once the real command
  replaces the shell. Captured in ~0.2 s after that.
- **`String.replace` reads `$` in the replacement as a literal `# Handoff — where the work stands
...
.** A
  scripted edit that inserted `echo $ > pidFile` via `.replace()` produced
  `echo $ >` in the file — writing a `# Handoff — where the work stands
...
error made a healthy case look broken at a glance.
 ...
 instead of the pid, which is why the
  first fix looked like it had not worked. Use `.split(a).join(b)`, or an Edit,
  when the replacement text may contain `# Handoff — where the work stands
...
. This bit an edit in THIS project;
...
over a dimmed banner, and `sleep 30 &` reports "Started in the background —
PID 38200. Nothing is capturing its output…".
